...
impl GraphicsEngine {
    ...
    pub fn render_frame(
        ...
        // 3. Render billboards (trails and effects)
        if !data.billboards.is_empty() {
            self.render_billboards(data.billboards)?;
        }
        
        // 4. Draw 3D scene (ECS entities rendered via Scene Manager)
    ...
    /// Render billboard trail quads
    ///
    /// Renders billboard quads generated by the trail system. Billboards are rendered
    /// as instanced quads with velocity-aligned orientation and additive blending.
    ///
    /// # Draw Ordering
    /// Billboards should be drawn after opaque geometry but can be drawn before or
    /// after other transparent objects depending on desired visual effect.
    ///
    /// # Prerequisites  
    /// - Billboard mesh pool must be created
    /// - begin_dynamic_frame() must be called first
    /// - Camera must be set up for billboard orientation calculations
    ///
    /// # Arguments
    ...
    pub fn render_billboards(&mut self, billboard_quads: &[systems::billboard::BillboardQuad]) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let camera = self.current_camera.as_ref()
            .ok_or("Camera must be set before rendering billboards")?;
        
        // Use billboard rendering system (analogous to text rendering)
        systems::billboard::render_billboards_from_pool(
            &mut self.pool_manager,
            &mut *self.backend,
            billboard_quads,
            camera,
