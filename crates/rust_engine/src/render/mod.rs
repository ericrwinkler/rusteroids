...
impl GraphicsEngine {
    ...
    pub fn render_frame(
        ...
        if !data.billboards.is_empty() {
            self.render_billboards_with_type(data.billboards, systems::dynamic::MeshType::Billboard)?;
        }
    ...
    /// Render billboard quads using a specific MeshType
    ///
    /// This method allows rendering billboards from different pools, enabling
    /// multiple billboard types with different textures/materials.
    /// Industry standard approach: separate pools for different particle types.
    ///
    /// # Arguments
    /// * `billboard_quads` - Slice of billboard quads to render
    /// * `mesh_type` - Specific billboard mesh type (e.g., BillboardBullet, BillboardExplosion)
    ///
    /// # Returns
    /// Result indicating successful billboard rendering
    pub fn render_billboards_with_type(
        &mut self,
        billboard_quads: &[systems::billboard::BillboardQuad],
        mesh_type: systems::dynamic::MeshType,
    ) -> Result<(), Box<dyn std::error::Error>> {
        ...
        // Use billboard rendering system with specific mesh type
        systems::billboard::render_billboards_from_pool_with_type(
            &mut self.pool_manager,
            &mut *self.backend,
            billboard_quads,
            camera,
